...
# if (PARAMset_num > 1000) {                                                    # this is related to psa_rna_rsm.R
#   predTBcases = sim_preds0[,PARAMset_num - 1000]; rm(sim_preds0)
# } else {
...
# ResultsDir_PSA_current = paste0("/n/holyscratch01/menzies_lab/Users/ylh202/IncipienTB/PSA_calib_reinfect/","paramset",PARAMset_num,"/")
ResultsDir_PSA_current = paste0("~/IncipienTB/PSA_main/paramset",PARAMset_num,"/")
...
get_psa_res2 = function(name, strategies){
  ...
  # # get mean eff (discounted QALY) for the country ----------------------------
  # eff.dQALY = get_mean_eff(country_res, strategy_nums = StrategyNums, reward_input, r = 0.03)
  # names(eff.dQALY) = c("No_testing", "IGRA_TB", "IGRA_RNA_TB")
  # 
  # # get mean eff (undiscounted QALY) for the country ----------------------------
  # eff.QALY = get_mean_eff(country_res, strategy_nums = StrategyNums, reward_input, r = 0)
  # names(eff.QALY) = c("No_testing", "IGRA_TB", "IGRA_RNA_TB")
  # 
  # # get mean eff (discounted LY) for the country ------------------------------------
  # reward_input2 = ifelse(names(reward_input) == "DEAD", 0, 1)
  # names(reward_input2) = names(reward_input)
  # eff.dLY = get_mean_eff(country_res, strategy_nums = StrategyNums, reward_input2, r = 0.03)
  # names(eff.dLY) = c("No_testing", "IGRA_TB", "IGRA_RNA_TB")
  # 
  # # get mean eff (undiscounted LY) for the country ------------------------------------
  # eff.LY = get_mean_eff(country_res, strategy_nums = StrategyNums, reward_input2, r = 0)
  # names(eff.LY) = c("No_testing", "IGRA_TB", "IGRA_RNA_TB")
  # 
  # # get mean cost (discounted cost) for each country --------------------------------------------
  # cost = get_mean_cost(country_res, strategy_nums = StrategyNums, cost_input, r = 0.03)
  # names(cost) = c("No_testing", "IGRA_TB", "IGRA_RNA_TB")
  # 
  # # scenario analysis for RNA cost --------------------------------------------
  # cost_input["TEST.RNA"] = 150
  # cost150 = get_mean_cost(country_res, strategy_nums = StrategyNums, cost_input, r = 0.03)
  # names(cost150) = c("No_testing", "IGRA_TB", "IGRA_RNA_TB") 
  # 
  # cost_input["TEST.RNA"] = 60
  # cost60 = get_mean_cost(country_res, strategy_nums = StrategyNums, cost_input, r = 0.03)
  # names(cost60) = c("No_testing", "IGRA_TB", "IGRA_RNA_TB") 
  # 
  # cost_input["TEST.RNA"] = 30
  # cost30 = get_mean_cost(country_res, strategy_nums = StrategyNums, cost_input, r = 0.03)
  # names(cost30) = c("No_testing", "IGRA_TB", "IGRA_RNA_TB") 
  # 
  # cost_input["TEST.RNA"] = 15
  # cost15 = get_mean_cost(country_res, strategy_nums = StrategyNums, cost_input, r = 0.03)
  # names(cost15) = c("No_testing", "IGRA_TB", "IGRA_RNA_TB") 
  # 
  # # # calculate NMB for each strategy -------------------------------------------
  # # NMB.No_screening = eff[["No_testing"]]*WTP - cost[["No_testing"]]
  # # NMB.IGRA_TB      = eff[["IGRA_TB"]]*WTP - cost[["IGRA_TB"]]
...
get_psa_res3 = function(name, strategies){
  ...
  # save cea result to RDS file -----------------------------------------------
  file.out2 = paste0(ResultsDir_PSA_current,"cea_",name,"_costItems.RDS")
  saveRDS(cea, file.out2)
...
# rerun = c("MAR", "MYS", "MDA", "GUY", "UKR", "SDN", "UZB", "FJI", "ROU", "LKA", "HKG", "KOR", "CHN", "SLV", "RUS", "YEM", "BRA", "CPV", "ECU", "VEN", "NIC", "DOM", "LTU", "PAN", "TWN", "COL", "URY", "HND", "ARG", "BLR", "IRQ", "BIH", "BLZ", "ARM", "GTM", "MEX", "BGR", "PRT", "SYR", "CHL", "TTO", "ALB",
#           "TUR", "POL", "IRN", "JPN", "LBN", "EGY", "CRI", "ESP")
...
# x = future_lapply(CountryNames[ctemp], function(x) get_psa_res(strategies = StrategyNums, name = x), future.seed = TRUE)
# x = future_lapply(CountryNames, function(x) get_psa_res3(strategies = StrategyNums, name = x))
...
# 
# # x = mclapply(CountryNames[ctemp], function(x) get_psa_res2(strategies = 0, name = x), mc.cores= numCores)
# 
# set.seed(NULL)
# names(x) = CountryNames[ctemp]

# if(PARAMset_num == 703){
#   get_psa_res4(strategies = StrategyNums, name = "HND")
# } else if(PARAMset_num == 719){
#   get_psa_res4(strategies = StrategyNums, name = "BRA")
# } else if (PARAMset_num == 266){
#   get_psa_res4(strategies = StrategyNums, name = "CHE")
# }
...
# x = mclapply(CountryNames, function(x) get_psa_res(strategies = StrategyNums, name = x), mc.cores = 8)
